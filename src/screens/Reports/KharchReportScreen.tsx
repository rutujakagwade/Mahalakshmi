...
export const KharchReportScreen: React.FC<KharchReportScreenProps> = ({
  ...
  const [expenses, setExpenses] = useState<ExpenseRowItem[]>([]);
  ...
  // Helper: detect machine kharch from notes or category/icon
  const isMachineKharch = (item: ExpenseRowItem): boolean => {
    ...
    return !!(
      ...
      item.iconType === 'labour' ||
      item.iconType === 'diesel' ||
      cat.includes('मजुरी') ||
      cat.includes('मजूर') ||
      cat.includes('पगार') ||
      cat.includes('labour') ||
      cat.includes('ड्रायव्हर') ||
      cat.includes('ऑपरेटर') ||
      notesStr.includes('मजूर') ||
      notesStr.includes('मजुरी') ||
      notesStr.includes('पगार') ||
      notesStr.includes('ऑपरेटर')
    );
  };
  ...
  const getCategoryAndIcon = (desc: string): { category: string; iconType: ExpenseRowItem['iconType'] } => {
    ...
    if (d.includes('मजुरी') || d.includes('मजूर') || d.includes('पगार') || d.includes('salary') || d.includes('labour') || d.includes('ड्रायव्हर') || d.includes('ऑपरेटर')) {
      return { category: 'मजुरी / ऑपरेटर', iconType: 'labour' };
    }
    if (d.includes('जेवण') || d.includes('food') || d.includes('चहा') || d.includes('नाश्ता') || d.includes('हॉटेल')) {
      return { category: 'जेवण', iconType: 'food' };
    }
    if (d.includes('साहित्य') || d.includes('material') || d.includes('पार्ट') || d.includes('ऑइल') || d.includes('सामग्री') || d.includes('दुरुस्ती') || d.includes('सर्व्हिसिंग')) {
      return { category: 'साहित्य / दुरुस्ती', iconType: 'material' };
    }
  ...
  const fetchExpenses = async () => {
    try {
      const [res, storedWorkers] = await Promise.all([
      ...
      // Also merge any worker payments stored in MajurYadi storage that aren't already in ledger
      if (storedWorkers) {
        try {
            ...
            workers.forEach((w: any) => {
              if (Array.isArray(w.payments)) {
                w.payments.forEach((p: any) => {
                  ...
                  const rawDate = formatToIsoDate(p.date);
                  const mName = p.machineName || w.assignedMachine || 'मजूर/ऑपरेटर';
                  parsed.push({
                    id: p.id || `worker_pay_${Math.random()}`,
                    category: 'मजुरी / ऑपरेटर',
                    displayDate: p.date ? (p.date.includes('-') ? formatToDisplayDate(p.date) : p.date) : formatToDisplayDate(rawDate),
                    rawDate: rawDate,
                    amount: Number(p.amount) || 0,
                    iconType: 'labour',
                    notes: `[machine: ${mName}] ${p.notes ? `${p.notes} - ` : ''}मजूर: ${w.name}${w.role ? ` (${w.role})` : ''}`,
                  });
                });
              }
            });
