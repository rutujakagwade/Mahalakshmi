...
import { sendWorkBalanceOnWhatsApp } from '../../utils/whatsapp';
...
export const ChaluKamListScreen: React.FC<ChaluKamListScreenProps> = ({
  ...
  const [works, setWorks] = useState<ChaluWorkItem[]>([]);
    ...
    try {
      ...
      if (rawList.length > 0) {
        const parsed: ChaluWorkItem[] = rawList.map((it: any) => {
          const tot = Number(it.totalAmount || it.amount) || 0;
          const adv = Number(it.advanceAmount || it.advance_amount) || 0;
          const bal = Number(it.balanceAmount || it.balance_amount) || Math.max(0, tot - adv);
          const wType = (it.workType === 'foot' || it.workType === 'theka') ? it.workType : (it.hoursUnit === 'trips' ? 'foot' : 'hours');
          const qty = Number(it.quantity || it.hoursOrTrips || it.hours_or_trips) || 0;
          const rt = Number(it.rate) || (qty > 0 && tot > 0 ? Math.round(tot / qty) : undefined);

          return {
            id: it.id || Math.random().toString(),
            workName: it.workName || it.workDescription || it.work_description || 'मशीन काम',
            customerName: it.customerName || it.customer?.name || 'थेट ग्राहक',
            customerPhone: it.customerPhone || it.customer?.phone || '',
            machineName: it.machineName || it.machine?.name || 'मशीन',
            village: it.village || it.location || '',
            workType: wType,
            rate: rt,
            quantity: qty,
            totalAmount: tot,
            ...
            startDate: it.entryDate || it.entry_date || it.date || 'चालू',
            status: it.status === 'completed' ? 'completed' : 'ongoing',
            notes: it.notes || '',
          };
        });
  ...
  const handleSaveEdit = async () => {
    ...
    setEditSaving(true);
    try {
      if (typeof selectedWork.id === 'number' || (typeof selectedWork.id === 'string' && !selectedWork.id.startsWith('w_'))) {
  ...
  const handleToggleStatus = async (item: ChaluWorkItem) => {
    const newStatus = item.status === 'ongoing' ? 'completed' : 'ongoing';
    try {
      if (typeof item.id === 'number' || (typeof item.id === 'string' && !item.id.startsWith('w_'))) {
        await MachineEntryService.update(item.id, { status: newStatus });
      }
      setWorks((prev) =>
      ...
      setToastMsg('स्टेटस बदलताना समस्या आली.');
    }
    setTimeout(() => setToastMsg(''), 2500);
  };
            ...
            {filteredWorks.map((work) => (
              <View key={work.id} style={styles.workCard}>
                ...
                <View style={styles.cardActionsRow}>
                  ...
                  {/* WhatsApp Share Button if balance pending */}
                  {work.balanceAmount > 0 ? (
                    <TouchableOpacity
                      style={[styles.actionBtn, styles.actionWhatsAppBtn]}
                      onPress={() => sendWorkBalanceOnWhatsApp({
                        customerName: work.customerName,
                        phone: work.customerPhone,
                        workName: work.workName,
                        machineName: work.machineName,
                        location: work.village,
                        totalAmount: work.totalAmount,
                        advanceAmount: work.advanceAmount,
                        balanceAmount: work.balanceAmount,
                        startDate: work.startDate,
                      })}
                      activeOpacity={0.7}
                    >
                      <Share2 size={13} color="#15803D" />
                      <Text style={[styles.actionBtnText, styles.actionWhatsAppText]}>
                        WhatsApp
                      </Text>
                    </TouchableOpacity>
                  ) : null}
