...
export const EditCustomerScreen: React.FC<EditCustomerScreenProps> = ({
    ...
    const fetchCustomer = async () => {
      try {
        ...
        if (cust) {
          ...
          setOpeningBalance(cust.opening_balance ? String(cust.opening_balance) : '0');
          setNotes(cust.notes || '');
        }
      } catch {
        Alert.alert('त्रुटी', 'ग्राहक माहिती लोड करताना समस्या आली.');
        onBack();
      } finally {
        setLoading(false);
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSavedMsg('ग्राहक तपशील अपडेट झाले!');
      setTimeout(() => {
        setSavedMsg('');
        if (onSuccess) {
          onSuccess();
      ...
      Alert.alert('त्रुटी', 'ग्राहक अपडेट करताना त्रुटी आली. कृपया पुन्हा प्रयत्न करा.');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <View style={styles.screen}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>ग्राहक संपादन</Text>
        </View>
